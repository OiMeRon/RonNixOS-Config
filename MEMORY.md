...
- **（2026-09-24）Clash Mi 声明式装入**（`modules/clashmi.nix`，模式 A）：本体 `Extracted/clashmi/`（1.0.30.1605，AppImage 解压），wrapper + desktop + 图标进 system profile；无 Linux TUN helper → 纯 `LD_LIBRARY_PATH`（比 gopeed 多 `libgcrypt`）。nixpkgs 的 `clashmi` 已于 2026-01-31 移除（unmaintained）
- **（2026-09-24）FlClash 卸载（用户裁定）**：与 Clash Mi 抢 `mixed-port: 7890`；声明与用户数据均已移除。**本机唯一代理客户端 = Clash Mi**
